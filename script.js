...
function showPage(page) {
  ...
  document.body.classList.toggle("long-scroll", page === 10 || page === 11);
  if (page === 10 || page === 11) {
...
function fadeOutBackgroundMusicThenPlayFinalSong() {
  ...
  const fadeInterval = setInterval(() => {
    ...
    if (volume <= 0) {
      ...
      if (finalSongAuto && !finalSongStarted) {
        finalSongStarted = true;
        finalSongAuto.volume = 1;
        finalSongAuto.play().catch(() => {});
      }
    }
  }, 220);
}
