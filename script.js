...
function preloadAppAssets() {
  ...
  const storyLines = [
    "Every love story begins with a memory...",
    "Finding our photos...",
    "Loading our little moments...",
    "Adding hearts and flowers...",
    "Almost ready, my love..."
  ];
  let lineIndex = 0;
  const lineTimer = window.setInterval(() => {
    if (!text) return;
    text.textContent = storyLines[lineIndex % storyLines.length];
    lineIndex += 1;
  }, 1150);
  if (preloader) preloader._lineTimer = lineTimer;
  const sources = Array.from(document.querySelectorAll("img, video[poster]"))
  ...
  const update = () => {
    ...
    if (text) text.textContent = percent >= 100 ? "Ready" : `Loading memories ${percent}%`;
  };

  if (!sources.length) {
    loaded = total;
    update();
    scheduleFinishPreloader(preloader, startedAt);
    return;
  }
  ...
  sources.forEach(src => {
    ...
    img.onload = img.onerror = () => {
      loaded += 1;
      update();
      if (loaded >= total) scheduleFinishPreloader(preloader, startedAt);
    };
    img.src = src;
  });

  // Safety fallback so the user never gets stuck if one asset is missing.
  window.setTimeout(() => scheduleFinishPreloader(preloader, startedAt), 9000);
}

function scheduleFinishPreloader(preloader, startedAt) {
  const elapsed = Date.now() - startedAt;
  const wait = Math.max(0, INTRO_MIN_DURATION - elapsed);
  window.setTimeout(() => finishPreloader(preloader), wait);
}

function finishPreloader(preloader) {
  document.body.classList.add("assets-ready");
  if (preloader && preloader._lineTimer) window.clearInterval(preloader._lineTimer);
  const text = document.getElementById("preloaderText");
  if (text) text.textContent = "A little surprise is waiting...";
  const fill = document.getElementById("preloaderFill");
...
function startBackgroundMusic() {
  ...
  if (playPromise !== undefined) {
    playPromise
      .then(() => {
        bgMusicStarted = true;
        document.body.classList.add("music-fade-in");
        let volume = 0;
        const fade = setInterval(() => {
          volume += 0.035;
          bgMusicAuto.volume = Math.min(volume, 0.82);
          if (volume >= 0.82) {
            clearInterval(fade);
            document.body.classList.remove("music-fade-in");
          }
        }, 90);
      })
      .catch(() => {
...
if (originalShowPageForMusicFade) {
  showPage = function(page, direction = "next") {
    ...
    document.body.classList.toggle("final-cinematic-active", page === 9);
    if (page === 9) {
      createFinalCinematicBurst();
    }
...
function createFinalCinematicBurst() {
  const page = document.getElementById("page9");
  if (!page) return;
  page.querySelectorAll(".final-burst").forEach(el => el.remove());
  const burst = document.createElement("div");
  ...
  for (let i = 0; i < 32; i++) {
    ...
    span.style.zIndex = "2";
    span.style.transition = `transform ${1100 + Math.random() * 700}ms ease, opacity ${1100 + Math.random() * 700}ms ease`;
    const angle = (Math.PI * 2 * i) / 32;
    const dist = 80 + Math.random() * 150;
    burst.appendChild(span);
    requestAnimationFrame(() => {
      span.style.transform = `translate(${Math.cos(angle) * dist}px, ${Math.sin(angle) * dist - 80}px) rotate(${Math.random() * 320}deg)`;
